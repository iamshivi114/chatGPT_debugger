...
class MyWidget extends Widget {
  ...
  private async getResponse(query: string): Promise<string> {
    const response = await fetch('http://localhost:5000/get_response', {
      ...
      // credentials: 'include' // add this option to include cookies and enable CORS
    });
...
const extension: JupyterFrontEndPlugin<void> = {
  ...
  activate: (app: JupyterFrontEnd, palette: ICommandPalette) => {
    console.log('JupyterLab extension chatgpt debugger is activated');
    ...
    const tracker = new WidgetTracker<Widget>({ namespace: 'my-extension' });
    tracker.add(widget);

    // Add the command to the palette.
    const command: string = 'my-extension:open';
